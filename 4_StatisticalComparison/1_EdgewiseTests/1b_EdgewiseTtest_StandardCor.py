...
fnirs_folder = r"D:\Foivos\fNIRS_fMRI_Study\fNIRS_data\1_CorrelationMatrices\1_StandardCor"
fmri_folder = r"D:\Foivos\fNIRS_fMRI_Study\fMRI_data\3_CorrelationMatrices\1_StandardCor"
...
for file in os.listdir(fnirs_folder):
    if file.endswith("_CrossCorM.xlsx"):
...
for file in os.listdir(fmri_folder):
    if file.endswith("_StandardCorM.xlsx"):
        subject_id = file.split("_")[0]
...
with pd.ExcelWriter(os.path.join(output_folder, f"Edgewise_CrossModality_2samplesTtest_StandCor_{modality}.xlsx")) as writer:
