"""
Revision: Eq. bounds range is 0.01 to 0.4 with step of 0.001
"""
import os
import pandas as pd
import numpy as np
...
# load the paths
fmri_folder_path = r"D:\Foivos\ResearchData_Archived\Light on Broken Networks\Light on Broken Networks Data\fMRI_data\3_CorrelationMatrices"
fnirs_folder_path = r"D:\Foivos\ResearchData_Archived\Light on Broken Networks\Light on Broken Networks Data\fNIRS_data\1_CorrelationMatrices"
...
# create the equivalence bounds array that will increase by 0.001 from min_bound to max_bound
equivalence_bounds = np.round(np.linspace(min_bound, max_bound, int(round((max_bound - min_bound) / 0.001)) + 1), 3)
# np.arange(min_bound, max_bound, 0.002).round(3)
print("equiv bounds\n", equivalence_bounds)
print("number of bounds:", len(equivalence_bounds))
...
#! TESTING SOMETHING
# Precompute once (same data, faster access)
fmri_partial_subjs = list(fmri_partial_data.keys())
fnirs_partial_subjs = list(fnirs_partial_data.keys())
fmri_bivariate_subjs = list(fmri_bivariate_data.keys())
fnirs_bivariate_subjs = list(fnirs_bivariate_data.keys())

fmri_partial_stack = np.stack([fmri_partial_data[s].to_numpy(dtype=float) for s in fmri_partial_subjs], axis=0)
fnirs_partial_hbo_stack = np.stack([fnirs_partial_data[s]["HbO"].to_numpy(dtype=float) for s in fnirs_partial_subjs], axis=0)
fnirs_partial_hbr_stack = np.stack([fnirs_partial_data[s]["HbR"].to_numpy(dtype=float) for s in fnirs_partial_subjs], axis=0)

fmri_bivariate_stack = np.stack([fmri_bivariate_data[s].to_numpy(dtype=float) for s in fmri_bivariate_subjs], axis=0)
fnirs_bivariate_hbo_stack = np.stack([fnirs_bivariate_data[s]["HbO"].to_numpy(dtype=float) for s in fnirs_bivariate_subjs], axis=0)
fnirs_bivariate_hbr_stack = np.stack([fnirs_bivariate_data[s]["HbR"].to_numpy(dtype=float) for s in fnirs_bivariate_subjs], axis=0)
#! TEST END 
import time
idx = 0
...
for bound in equivalence_bounds:
    ...
    idx += 1
    print(f"Performing TOST for bound: {bound} [{idx}/{len(equivalence_bounds)}]")
    ...
    for i in range(len(template_df.index)): # rows
        for j in range(i+1, len(template_df.columns)): # columns
            ...
            fmri_par_edge_values = fmri_partial_stack[:, i, j]
            fnirs_par_edge_values_hbo = fnirs_partial_hbo_stack[:, i, j]
            fnirs_par_edge_values_hbr = fnirs_partial_hbr_stack[:, i, j]
            ...
            fmri_biv_edge_values = fmri_bivariate_stack[:, i, j]
            fnirs_biv_edge_values_hbo = fnirs_bivariate_hbo_stack[:, i, j]
            fnirs_biv_edge_values_hbr = fnirs_bivariate_hbr_stack[:, i, j]
    ...
    end_bound = time.perf_counter()
    print(f"    Finished bound {bound:.3f} in {end_bound - start_bound:.3f} s")

  
    
# save the results to excel files now
output_folder = r"C:\Users\foivo\Desktop\fNIRS_fMRI_Study\3_CrossModality_ttost_revised_faster"
